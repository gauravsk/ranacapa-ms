---
title: 'ranacapa: An R package to explore environmental DNA data with exploratory
  statistics and interactive visualizations'
author: Gaurav S. Kandlikar (1,+), Zachary J. Gold (1), Madeline C. Cowen (1), Rachel
  S. Meyer (1), Amanda C. Freise (2), Nathan J.B. Kraft (1), Jordan Moberg-Parker
  (2), Joshua Sprague (3), David Kushner (3), and Emily E. Curd (1)
fontsize: 11pt
output: 
  word_document:
    reference_docx: rendering-templates/word_template.docx
  rtf_document:
header-includes: \usepackage{color} \usepackage{lineno} \linenumbers \linespread{1.5}
  \usepackage{float} \usepackage[table]{xcolor} \usepackage{setspace} \usepackage{amsmath}
  \numberwithin{equation}{section} \usepackage{breqn} \usepackage{booktabs} \usepackage{parskip}
  \usepackage{tabularx} \setlength{\parindent}{5ex} \usepackage{sectsty} \paragraphfont{\normalfont\itshape}
fontfamily: mathpazo
urlcolor: blue
...
The second required input is a sample metadata file uploaded as a `.txt` file. The only requirements for this metadata file are that the entries in the first column of this file should match the column names in the sample names in the taxonomy table. A valid metadata file for the taxonomy table above would be as follows:
